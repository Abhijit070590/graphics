...
float maxZ = -100.0, maxX = -100.0, maxY = -100.0, minX = 0.0, minY = 0.0, minZ = 0.0;
...
void draw()
{
	...
	GLfloat scale = 0.1f;
	GLfloat scalex = 1.0/(maxX - minX), scaley=1.0/(maxY-minY), scalez=1.0/(maxZ-minZ) ;
	...
	for(int j=0;j<number_of_faces;j++){
			...
			glBegin(GL_TRIANGLES);
				...
				Ydiff = (vertices[ver1].y - Yinc) * scale;
				//glTexCoord2f((vertices[ver1].x - Xinc) * scale,Ydiff);
				glTexCoord3f((vertices[ver1].x - minX) * scalex, (vertices[ver1].y - minY) * scaley,
				(vertices[ver1].z-minZ)*scalez);//Ydiff);
				...
				Ydiff = (vertices[ver2].y - Yinc) * scale;
				//glTexCoord2f((vertices[ver2].x - Xinc) * scale, Ydiff);
				glTexCoord3f((vertices[ver2].x - minX) * scalex, (vertices[ver2].y - minY) * scaley,
				(vertices[ver2].z-minZ)*scalez);
				...
				Ydiff = (vertices[ver3].y - Yinc) * scale;
				//glTexCoord2f((vertices[ver3].x - Xinc) * scale, Ydiff);
				glTexCoord3f((vertices[ver3].x - minX) * scalex, (vertices[ver3].y - minY) * scaley,
				(vertices[ver3].z-minZ)*scalez);//Ydiff);
...
void render()
{
			...
			Rotation_by_quaternions(yaw / 10, 0, 1, 0);
			
			Rotation_by_quaternions(pitch / 10, 1, 0, 0);
