...
void Mesh::createEdgePairs()
{
	...
	for(iter1 = _edgeList.begin(); iter1 != _edgeList.end(); iter1++)
	{
		...
		if(edge->twin == NULL)
		{
			...
			{
				e_twin = *iter2;
				if(edge->vert == e_twin->prev->vert && edge->prev->vert == e_twin->vert){
					edge->twin = e_twin;
					e_twin->twin = edge;
					break;
				} //if
...
vector<int> Mesh::oneRingNbs(int vIndx) const
{
	...
	{
		e_twin = edge->twin;
		ngh_indices.push_back(e_twin->vert->index);
		edge = e_twin->prev;
	}while (edge != e0);
	return ngh_indices;
}
...
void Mesh :: subdivide(){
		...
		for(e_iter = _edgeList.begin(); e_iter != _edgeList.end(); e_iter++) (*e_iter)->twin = NULL;
